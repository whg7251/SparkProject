package com.guo.spark.skynet;

import java.util.*;

import com.guo.spark.conf.ConfigurationManager;
import com.guo.spark.dao.IMonitorDAO;
import com.guo.spark.dao.ITaskDAO;
import com.guo.spark.dao.factory.DAOFactory;
import com.guo.spark.domain.MonitorState;
import com.guo.spark.domain.Task;
import com.guo.spark.domain.TopNMonitor2CarCount;
import com.guo.spark.domain.TopNMonitorDetailInfo;
import com.guo.spark.util.ParamUtils;
import com.guo.spark.util.SparkUtils;
import com.guo.spark.util.StringUtils;
 ...
 *
 */
public class TopNCarSpeedMonitorFlowAnalyze {
...
//				 sc.accumulator("", new MonitorAndCameraStateAccumulator());
       ...
       /* JavaPairRDD<String, Row> filter = monitor2DetailRDD.filter(new Function<Tuple2<String, Row>, Boolean>() {
            @Override
            public Boolean call(Tuple2<String, Row> tp) throws Exception {
                String monitorId = tp._1;
                List<String> value = broadcast.value();
                return value.contains(monitorId);
            }
        });
        JavaPairRDD<String, Iterable<Row>> groupByKey = filter.groupByKey();
        groupByKey.foreach(new VoidFunction<Tuple2<String, Iterable<Row>>>() {
            @Override
            public void call(Tuple2<String, Iterable<Row>> tp) throws Exception {
                String monitorId = tp._1;
                Iterator<Row> iter = tp._2.iterator();

                Row[] top10 = new Row[10];

                while(iter.hasNext()){

                    Row currentRow = iter.next();
                    Integer currentSpeed = Integer.valueOf(currentRow.getAs("speed")+"");
                    for(int i = 0;i<top10.length;i++){
                        if(top10[i]==null){
                            top10[i] = currentRow;
                            break;
                        }else if(currentSpeed>Integer.valueOf(top10[i].getAs("speed")+"")){
                            for(int j = 9;j>i;j--) {
                                top10[j] = top10[j-1];
                            }
                            top10[i] = currentRow;
                ...
                for(Row row: top10){
                    System.out.println("monitorId " + monitorId+",car = "+row.getAs("car")+",speed = "+row.getAs("speed"));
                }
